...
public class EntityUtil
    ...
    public static ArrayList<BlockPos> getPos(double posX, double posY, double posZ, Entity entity) {
        ...
        if (entity != null) {
            final AxisAlignedBB bb = entity.ridingEntity != null ? entity.ridingEntity.getEntityBoundingBox().contract(0.0d, 0.0d, 0.0d).offset(posX, posY, posZ) : entity.getEntityBoundingBox().contract(0.000000000000999999999999999999d, 1d, 0.000000000000999999999999999999d).offset(posX, posY, posZ);
            int y = (int) bb.minY;
    ...
    public static boolean isEntityAlive(final Entity entity) {
    ...
    public static int getPing(final EntityPlayer player) {
        try {
            if (player != null && mc.player.connection != null) {
                return Objects.requireNonNull(mc.player.connection.getPlayerInfo(player.getUniqueID()).getResponseTime());
            }
            return -1;
        } catch (Exception e) {
            return -1;
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && PutaHacknn.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && PutaHacknn.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (PutaHacknn.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2\u00a7f0", (int) EntityUtil.mc.player.getDistance(player));
