...
import me.dev.putahacknn.util.BebRenderUtil;
...
public class Speedmine extends Module
{
    ...
    public Speedmine() {
        super("Speedmine", "speedy gonzales", Category.PLAYER, true, false, false);
    ...
    public void onRender3D(final Render3DEvent event) {
        if (Speedmine.mc.player != null && Speedmine.mc.world != null && this.render.getValue() && this.breakPos != null && Speedmine.mc.world.getBlockState(this.breakPos).getBlock() != Blocks.AIR) {
            ...
            final float breakTime = Speedmine.mc.world.getBlockState(this.breakPos).getBlockHardness((World)Speedmine.mc.world, this.breakPos);
            final double progression = this.timer.getPassedTimeMs() / 40.0f / breakTime * PutaHacknn.serverManager.getTpsFactor();
            ...
            if (this.renderMode.getValue().equals(RenderMode.RISE)) {
                if (riseBB.maxY <= oldMaxY) {
                    BebRenderUtil.drawFilledBox(riseBB, ((boolean)this.statusColor.getValue()) ? color.getRGB() : renderColors.getRGB());
                    BebRenderUtil.drawBlockOutline(riseBB, ((boolean)this.statusColor.getValue()) ? color : renderColors, 1.0f);
                }
                if (riseBB.maxY >= oldMaxY) {
                    BebRenderUtil.drawFilledBox(bb, ((boolean)this.statusColor.getValue()) ? color.getRGB() : renderColors.getRGB());
                    BebRenderUtil.drawBlockOutline(bb, ((boolean)this.statusColor.getValue()) ? color : renderColors, 1.0f);
                }
            }
            else if (this.renderMode.getValue().equals(RenderMode.GROW)) {
                if (axisAlignedBB1.maxY <= oldMaxY) {
                    BebRenderUtil.drawFilledBox(axisAlignedBB1, ((boolean)this.statusColor.getValue()) ? color.getRGB() : renderColors.getRGB());
                    BebRenderUtil.drawBlockOutline(axisAlignedBB1, ((boolean)this.statusColor.getValue()) ? color : renderColors, Float.intBitsToFloat(Float.floatToIntBits(7.8420115f) ^ 0x7F7AF1C2));
                }
                if (axisAlignedBB1.maxY >= oldMaxY) {
                    BebRenderUtil.drawFilledBox(bb, ((boolean)this.statusColor.getValue()) ? color.getRGB() : renderColors.getRGB());
                    BebRenderUtil.drawBlockOutline(bb, ((boolean)this.statusColor.getValue()) ? color : renderColors, Float.intBitsToFloat(Float.floatToIntBits(117.64367f) ^ 0x7D6B498F));
                }
            }
                ...
                BebRenderUtil.drawFilledBox(bb, ((boolean)this.statusColor.getValue()) ? new Color(color.getRed(), color.getGreen(), color.getBlue(), (int)(fadeA * 255.0f)).getRGB() : alphaFade.getRGB());
                BebRenderUtil.drawBlockOutline(bb, ((boolean)this.statusColor.getValue()) ? new Color(color.getRed(), color.getGreen(), color.getBlue(), (int)(fadeA * 255.0f)) : alphaFade, 1.0f);
            }
            else if (this.renderMode.getValue().equals(RenderMode.ANIMATE)) {
                BebRenderUtil.drawFilledBox(BebRenderUtil.fixBB(new AxisAlignedBB(this.breakPos)).shrink((double)shrinkFactor), ((boolean)this.statusColor.getValue()) ? color.getRGB() : renderColors.getRGB());
                BebRenderUtil.drawBlockOutline(BebRenderUtil.fixBB(new AxisAlignedBB(this.breakPos)).shrink((double)shrinkFactor), ((boolean)this.statusColor.getValue()) ? color : renderColors, Float.intBitsToFloat(Float.floatToIntBits(119.3883f) ^ 0x7D6EC6CF));
            }
        }
    }
